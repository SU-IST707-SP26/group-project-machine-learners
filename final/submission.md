...
**Company scale is the second consistent driver.** `log_total_assets`, which is the metric used to describe compnay size, (labeled "big_total_assets" in the figure) carries a positive coefficient across Total Score, Environment, and Social targets. `log_public_float` also appears across Total Score and Social targets. Larger, more widely-held companies receive higher ESG scores. This could be a result of their greater capacity to invest in ESG programs, and the rating agencies' tendency to cover large-cap firms more thoroughly.
